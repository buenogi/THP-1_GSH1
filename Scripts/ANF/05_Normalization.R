...
DataANF <-  read.csv(file = "Data/processed/DataANF_viability.csv")

# Normalizing basis

REF <- filter(DataANF, pop == "REF")
C6 <- filter(DataANF, pop == "C6")
C7 <- filter(DataANF, pop == "C7")
C44 <- filter(DataANF, pop == "C44")
C58 <- filter(DataANF, pop == "C58")
C67 <- filter(DataANF, pop == "C67")
C73 <- filter(DataANF, pop == "C73")
C85 <- filter(DataANF, pop == "C85")
C76 <- filter(DataANF, pop == "C76")
C67p <- filter(DataANF, pop == "C67p")
C68 <- filter(DataANF, pop == "C68")
...
DataANF_Full  <- rbind(REF,C6,C7,C44,C58, C67,C73,C85, C76,C67p,C68)


# CSV exportation

write.csv(DataANF_Full , file = "Data/processed/DataANF_processed_normalized.csv", 
          sep = ",", row.names = F)
