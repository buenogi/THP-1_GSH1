...
DataMIL <-  read.csv(file = "Data/processed/DataMIL_viability.csv")
...
C76 <- filter(DataMIL, pop == "C76")
C67p <- filter(DataMIL, pop == "C67p")
...
DataMIL_Full  <- rbind(REF,C6,C7,C44,C58, C67,C73,C85, C76,C67p)


# CSV exportation

write.csv(DataMIL_Full , file = "Data/processed/DataMIL_processed_normalized.csv", 
          sep = ",", row.names = F)
